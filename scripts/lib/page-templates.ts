/**
 * Shared page templates for the multi-page setup.
 *
 * Both `scripts/seo-inject.ts` (build-time) and the Vite dev plugin
 * (`scripts/dev-pages-plugin.ts`, dev-time) import these so the static
 * detail pages render identically whether you're running `npm run dev`
 * or visiting the production site.
 *
 * Don't put any file-system operations in here — keep it pure HTML
 * generation so the dev plugin can call it on every request without
 * touching disk.
 */
...
// ─── Constants ─────────────────────────────────────────────────────────────
export const SITE = "https://syso.codes";

export const CATEGORIES: Record<string, { label: string; description: string }> = {
  "audio-server": {
    label: "Audio servers",
    description:
      "Streaming and audio server projects — Icecast-compatible, JACK-driven, network-audio-aware.",
  },
  "audio-app": {
  ...
  fpga: {
    label: "FPGA gateware",
    description:
      "Lattice ECP5 / Colorlight i9 gateware in Verilog and LiteX/Migen — AES67, Ableton Link in HW, etc.",
  },
  "linux-audio": {
    label: "Linux audio",
  ...
  "ai-tools": {
    label: "AI tools",
    description:
      "MCP servers, agentic workflows, and AI-assisted DSP / transcription / translation tooling.",
  },
  "dev-tools": {
...
export const sorted: Project[] = [...merged].sort((a, b) => {
...
export const grouped: Record<string, Project[]> = {};
for (const p of sorted) {
  (grouped[p.category] ??= []).push(p);
}

export const projectsBySlug: Map<string, Project> = new Map(
  sorted.map((p) => [p.slug, p])
);
...
export function projectUrl(p: Project) {
  return `${SITE}/projects/${p.slug}/`;
}
export function categoryUrl(c: string) {
...
const STATIC_CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; }
  ...
  /* breadcrumb */
  .crumbs { display: flex; gap: 8px; align-items: center; margin: 32px 0 24px; font-size: 13px; color: #888; flex-wrap: wrap; }
...
function pageNav() {
  ...
  <a class="page-nav__brand" href="/" aria-label="syso.codes home">
...
export function projectPage(p: Project): string {
  ...
  <div class="crumbs" aria-label="Breadcrumb">
    ...
    <a href="/projects/">projects</a>
    <span class="sep">/</span>
    <a href="/categories/${p.category}/">${escHtml(categoryLabel(p.category))}</a>
    <span class="sep">/</span>
  ...
  <div class="cta-row">
    ...
    <a class="btn btn-ghost" href="/categories/${p.category}/">
  ...
  ${
    p.image
      ? `<div class="media"><img src="${escAttr(p.image)}" alt="${escAttr(p.name)} screenshot" loading="lazy" referrerpolicy="no-referrer" /></div>`
      : ""
  }
...
export function categoriesIndexPage(): string {
  const title = "All categories · syso.codes — by DatanoiseTV";
  const description =
    "Browse the open-source projects on syso.codes by category — embedded, FPGA, audio apps, audio servers, Linux audio, AI tools and developer libraries.";
...
${cats
  .map(
    ([key, list]) => `    <a href="/categories/${key}/">
...
export function projectsIndexPage(): string {
...
// ─── Homepage JSON-LD + noscript injection ─────────────────────────────────
export function homepageItemListJsonLd(): string {
  const itemList = {
    "@context": "https://schema.org",
    "@type": "ItemList",
...
export function homepageNoscript(): string {
  const inner = `
<style>
...
${list
  .map(
    (p) => `    <li>
      <a href="/projects/${p.slug}/" rel="noopener">${escHtml(p.name)}</a>
...
export function sitemapXml(): string {
  ...
  const urls = [
    ...
    { loc: `${SITE}/projects/`, priority: "0.9", changefreq: "weekly" as const },
    { loc: `${SITE}/categories/`, priority: "0.8", changefreq: "weekly" as const },
    { loc: `${SITE}/#about`, priority: "0.7", changefreq: "monthly" as const },
    { loc: `${SITE}/#activity`, priority: "0.7", changefreq: "daily" as const },
    { loc: `${SITE}/#featured`, priority: "0.8", changefreq: "weekly" as const },
    ...Object.keys(grouped)
      .filter((k) => CATEGORIES[k])
      .map((k) => ({
        loc: categoryUrl(k),
