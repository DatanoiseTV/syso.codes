...
export const sorted = [...merged].sort((a, b) => {
...
export const projectsBySlug = new Map(sorted.map((p) => [p.slug, p]));
// ─── Helpers ───────────────────────────────────────────────────────────────
export const escHtml = (s) => s
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;")
    .replace(/"/g, "&quot;")
...
export function projectUrl(p) {
    return `${SITE}/projects/${p.slug}/`;
}
export function categoryUrl(c) {
...
function htmlShell({ title, description, canonical, ogImage, jsonLd, body, breadcrumbs, }) {
...
export function categoriesIndexPage() {
  ...
  <div class="cat-grid">
${cats
        .map(([key, list]) => `    <a href="/categories/${key}/">
...
export function homepageItemListJsonLd() {
    const itemList = {
        ...
        description: "All featured open-source projects on syso.codes, sorted by most recent push.",
...
export function homepageNoscript() {
    const inner = `
<style>
...
${Object.entries(grouped)
        ...
        .map(([k, list]) => `  <h2>${escHtml(CATEGORIES[k].label)}</h2>
  <ul>
${list
        .map((p) => `    <li>
